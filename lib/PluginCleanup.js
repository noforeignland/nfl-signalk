...
class PluginCleanup {
  ...
  async removeOldPlugins(configDir) {
    ...
    return new Promise((resolve) => {
      setTimeout(async () => {
        const result = await this.attemptRemoval(foundOldPlugins, configDir);
        resolve(result);
  ...
  /**
   * Attempt to remove old plugin directories
   */
  async attemptRemoval(foundOldPlugins, configDir) {
